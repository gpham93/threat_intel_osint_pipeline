...
An enterprise-grade defense intelligence platform combining **BFO & CCO Formal Ontologies**, **PySpark & Splink Probabilistic Identity Resolution**, **Event-Driven Ingestion Pipelines**, **STIX 2.1 CTI Exchange**, and a **Zero-Hallucination GraphRAG Conversational Engine** with **Natural Language Inference (NLI) Entailment Proofs**.
...
- **Class Subsumptions**:
  $$\text{bfo:Continuant} \implies \text{cco:Agent} \implies (\text{cco:Person} \mid \text{cco:Organization})$$
...
- **OWL2 Existential Property Restrictions**:
  Every `MoneyTransfer` instance is constrained by existential agent attribution:
...
- **Comparison Vector Probability**:
  Given comparison vector $\gamma = (\gamma_{\text{name}}, \gamma_{\text{country}}, \gamma_{\text{reg\_id}})$, match probability $P(M | \gamma)$ is computed via:
...
- **Pathfinder Tool**: Calculates Breadth-First Search (BFS) shortest paths between any two threat entities ($v_{\text{start}} \to \dots \to v_{\text{end}}$).
